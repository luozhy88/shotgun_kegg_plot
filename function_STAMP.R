...
pathway_pdf=function(meta_info=meta_info,pathway_df=pathway_df,meta_col=meta_col,out.name=out.name){
    ...
    for (j in 1:nrow(pair)) {
      ...
      ggsave(filename = glue::glue("output/", out.name, comparison, "_metagenomics_smallSize.pdf"), width = 8, height = 5.5)
      ggsave(filename = glue::glue("output/", out.name, comparison, "_metagenomics_mediumSize.pdf"), width = 8.5, height = 7.0)
      ggsave(filename = glue::glue("output/", out.name, comparison, "_metagenomics_bigSize.pdf"), width = 8.5, height = 8.0)
    
      #  }
    }

}
pathway_pdf(meta_info=meta.348,pathway_df=kegg.pathway2,meta_col=c("sampleID", "Group", "Gender"),out.name="KEGG_")
############################################################################################################
